export const ASSET_TABLES_MISSING_MESSAGE = "자산관리 테이블이 아직 준비되지 않았습니다. supabase/finance.sql과 supabase/recurring_payments_update.sql을 실행해주세요.";
export const ASSET_LOAD_ERROR_MESSAGE = "자산관리 데이터를 불러오지 못했습니다.";

export const ASSET_TRANSACTION_COLUMNS = "id,type,title,amount,category,memo,payment_method,status,transaction_date,work_session_id,source,subscription_id,payment_date,created_at,updated_at";
...
export const SUBSCRIPTION_COLUMNS = "id,service_name,amount,billing_cycle,next_billing_date,category,memo,is_active,calendar_event_id,last_processed_date,created_at,updated_at";
...
export function mapAssetTransaction(row) {
  return { id: row.id, type: row.type, title: row.title || "", amount: Number(row.amount) || 0, category: row.category || "기타", memo: row.memo || "", paymentMethod: row.payment_method || "", status: row.status || "paid", transactionDate: row.transaction_date, workSessionId: row.work_session_id || null, source: row.source || null, subscriptionId: row.subscription_id || null, paymentDate: row.payment_date || null, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export function mapSubscription(row) {
  return { id: row.id, serviceName: row.service_name || "", amount: Number(row.amount) || 0, billingCycle: row.billing_cycle || "monthly", nextBillingDate: row.next_billing_date, category: row.category || "구독", memo: row.memo || "", isActive: row.is_active !== false, calendarEventId: row.calendar_event_id || null, lastProcessedDate: row.last_processed_date || null, createdAt: row.created_at, updatedAt: row.updated_at };
}

export function getKoreaDateValue(date = new Date()) {
  return new Intl.DateTimeFormat("en-CA", { year: "numeric", month: "2-digit", day: "2-digit", timeZone: "Asia/Seoul" }).format(date);
}

export function isSubscriptionDue(subscription, today = getKoreaDateValue()) {
  return Boolean(subscription?.isActive && subscription.nextBillingDate && subscription.nextBillingDate <= today && subscription.lastProcessedDate !== subscription.nextBillingDate);
}

export function advanceRecurringDate(dateValue, cycle) {
  ...
  const targetMonth = cycle === "yearly" ? month : month === 12 ? 1 : month + 1;
...
export function isAssetTableMissingError(error) {
  ...
  return ["42P01", "42703", "PGRST202", "PGRST204"].includes(error?.code) || ["finance_settings", "finance_transactions", "finance_categories", "work_sessions", "subscriptions", "recurring_payment_logs", "process_recurring_payment"].some((name) => message.includes(name));
}
