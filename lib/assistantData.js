...
export function getQuestionScopes(message) {
  ...
  const asksCalendar = /(일정|캘린더|스케줄|약속|회의|미팅)/.test(message);
  ...
  if (asksCalendar || (hasDateOnly && !asksAssets && !asksTasks)) scopes.push("calendar");
  return scopes.length ? scopes : ["assets", "tasks", "calendar"];
}
...
export async function loadAssetsSummary(supabase, userEmail, now = new Date()) {
  ...
  return {
    ...
    recentExpenses: transactions.filter((transaction) => transaction.type === "expense").slice(0, 10).map(toSafeTransaction),
