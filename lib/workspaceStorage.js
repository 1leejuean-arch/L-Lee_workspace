...
export async function updateTaskInSupabase(_userEmail, task) {
  ...
  return normalizeWorkspaceTask(data.task || task);
}

export async function updateTaskStepsInSupabase(_userEmail, taskId, steps) {
  const data = await requestJson("/api/tasks", {
    method: "PATCH",
    body: JSON.stringify({
      id: taskId,
  ...
  return normalizeWorkspaceTask(data.task || { id: taskId, steps });
}
...
export async function updateNoteInSupabase(_userEmail, note, unlockToken = null) {
  const data = await requestJson("/api/notes", {
    method: "PATCH",
    body: JSON.stringify({
      id: note.id,
...
export async function deleteNoteFromSupabase(_userEmail, noteId, unlockToken = null) {
  await requestJson("/api/notes", {
    method: "DELETE",
    body: JSON.stringify({ id: noteId, unlockToken }),
  });
}

export async function lockNoteInSupabase(noteId, password) {
  const data = await requestJson(`/api/notes/${noteId}/lock`, {
    method: "POST",
...
export async function unlockNoteInSupabase(noteId, password) {
  return requestJson(`/api/notes/${noteId}/unlock`, {
    method: "POST",
    body: JSON.stringify({ password }),
  });
}

export async function changeNotePasswordInSupabase(noteId, currentPassword, newPassword) {
  await requestJson(`/api/notes/${noteId}/lock`, {
    method: "PATCH",
    body: JSON.stringify({ currentPassword, newPassword }),
  });
}

export async function removeNoteLockInSupabase(noteId, currentPassword) {
  const data = await requestJson(`/api/notes/${noteId}/lock`, {
    method: "DELETE",
