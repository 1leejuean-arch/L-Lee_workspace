...
export function parseAssistantAction(message, { now = new Date() } = {}) {
  ...
  const hasCreateVerb = /(추가|등록|넣어|만들어|생성|기록해|적어|메모해)/.test(compact);
  ...
  else if (/(할일|해야할일)/.test(compact) && hasCreateVerb) intent = "create_task";
...
export function parseNaturalTime(message) {
  ...
  const plain = message.match(/(?:^|\s)([01]?\d|2[0-3])\s*시(?:\s*(\d{1,2})\s*분)?(?=\s|$|부터|에)/);
...
function cleanActionTitle(message, kind) {
  ...
  value = value
    ...
    .replace(/(?:오전|오후)\s*\d{1,2}\s*시(?:\s*\d{1,2}\s*분)?|(?:^|\s)(?:[01]?\d|2[0-3]):[0-5]\d|(?:^|\s)(?:[01]?\d|2[0-3])\s*시(?:\s*\d{1,2}\s*분)?/g, " ")
    .replace(/시간은\s*너가\s*원하는대로|시간\s*없음|부터\s*해서|부터해서/g, " ")
    .replace(/^\s*에\s*|\s+에\s+/g, " ");
  const patterns = {
    task: /할일|해야할일|추가해줘|추가해주세요|추가|등록해줘|등록|넣어줘|넣어|해줘|해주세요/g,
